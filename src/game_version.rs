...
use crate::minecraft::template_models::{column, cube_all, door_bottom_left, door_bottom_right, door_top_left, door_top_right, missing, orientable, pressure_plate_down, pressure_plate_up, torch, wall_torch};
...
fn load_b173(registry: &mut Registry) {
    ...
    iter_files_recursive(read_dir.clone().into(), &mut |entry| {
        if entry.file_name().to_str().unwrap().to_owned().ends_with(".json") {
            match serde_json::from_str::<MCModelFile>(fs::read_to_string(entry.path()).unwrap().as_str()) {
                Ok(model_file) => {
                    let resource_path = entry.path().file_stem().unwrap().to_string_lossy().replace(&read_dir, "");
                    let resource_id = Identifier::new(namespace.to_string(), resource_path);
                    model_files.insert(resource_id, model_file);
                },
                Err(e) => { eprintln!("Error processing {}: {}", entry.file_name().to_string_lossy(), e) }
            };
        }
    });
...

    {
        ...
        let property_list: &[(&str, &[&str])] = &[
            ...
            ("minecraft:redstone_side", &["down", "side"]),
    ...
    let block_register_list = vec![
        ...
        for block in block_register_list {
            block.map_states(registry);
            registry.get_block_register_mut().insert(block);
        }

        let missing_model = model_files.get(&Identifier::from_str("minecraft:missing")).expect("Did not have a missing model");

        for state in registry.get_blockstate_register().get_elements() {
            let identifier = state.get_property_identifier().get_identifier();
            ...
            let blockstate_model = match &blockstate_file {
                MCBlockstateType::variants(variants) => {
                    ...
                    let model = match model_files.get(&t_variant_model) {
                        Some(model_file) => {
